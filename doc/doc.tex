...
\begin{document}
	...
	\section{Forecasting the Discharge for the River of Bosna}
      ...
      In this study, three data-driven modeling approaches were evaluated for predicting the monthly mean river discharge (\emph{proticaj}) of the Bosna River: Random Forest (RF), Extreme Gradient Boosting (XGBoost), and Long Short-Term Memory neural networks (LSTM).
      
      Random Forest is an ensemble learning method based on bootstrap aggregation of decision trees. Due to its robustness to noise, ability to model nonlinear relationships, and limited sensitivity to multicollinearity, RF is frequently applied in hydrological modeling and is considered a strong baseline approach.
      ...
      Meteorological predictors include the mean monthly precipitation ($P$) and mean monthly air temperature ($T$), observed at five meteorological stations located within the Bosna River basin: Bjelašnica, Sarajevo, Zenica, Tuzla, and Doboj.
      
      The target variable is the monthly mean river discharge ($Q$), measured at the basin outlet. All variables were aggregated to monthly resolution to ensure temporal consistency and long-term coverage.
      ...
      Model performance was evaluated using hydrologically relevant efficiency metrics.
      ...
      \subsection{Hyperparameter Tuning of XGBoost}
      
      XGBoost hyperparameters were optimized using grid search combined with time-series cross-validation. The following parameters were considered:
      \begin{itemize}
      ...
      For each parameter combination, model performance was evaluated using the mean NSE across the five validation folds. The parameter set yielding the highest NSE was selected as optimal.
     ...
     \textit{Discussion}.    The performance metrics presented in Table~\ref{tab:model_performance} reveal notable differences in both fitting capacity and generalization ability among the evaluated models.
      ...
      \subsection{Feature Importance and Explainable Artificial Intelligence}
      ...
      The autoregressive discharge term ($Q_{t-1}$) also emerges as a key predictor, highlighting the strong persistence of monthly river flow and confirming the importance of short-term memory in discharge dynamics. Temperature-related features exhibit comparatively lower but non-negligible influence, likely capturing seasonal snowmelt and evapotranspiration effects, especially for upstream stations such as Bjelašnica.
      
      Notably, feature importance rankings remain broadly consistent across XGBoost and Random Forest, suggesting stable hydro-climatic controls on discharge. In contrast, the LSTM model displays weaker and less structured importance patterns, consistent with its overall lower predictive performance.
      
      
      The agreement between data-driven feature importance and established hydrological understanding provides confidence in the physical plausibility of the models. The dominance of lagged precipitation and discharge terms aligns with conceptual rainfall--runoff processes, while the secondary role of temperature reflects its indirect influence at monthly time scales.
      ...
      \begin{figure}[ht]
      	...
      	\caption{Direction and magnitude of the five most influential features identified through explainable artificial intelligence analysis:
      		...
      		(b) Random Forest,
      		and (c) LSTM.
      		Bars indicate the decrease in Nash--Sutcliffe Efficiency (NSE) after feature permutation, while color denotes the direction of influence (positive or negative contribution to predicted discharge).}
